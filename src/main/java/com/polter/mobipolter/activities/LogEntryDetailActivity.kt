...
import com.polter.mobipolter.activities.adapters.TabDetailPagerAdapter
...
import com.polter.mobipolter.activities.viewmodel.LogDetailViewModel
import jxl.CellView
import jxl.Workbook
...
import kotlinx.android.synthetic.main.layout_logdetail_item.view.*
import kotlinx.android.synthetic.main.layout_logdetail_item.view.txtDate
import kotlinx.android.synthetic.main.layout_logdetail_item.view.txtKind
import kotlinx.android.synthetic.main.layout_logdetail_item.view.txtLength
import kotlinx.android.synthetic.main.layout_logdetail_item.view.txtLogTitle
import kotlinx.android.synthetic.main.layout_logdetail_item.view.txtQuality
import kotlinx.android.synthetic.main.layout_logdetail_item.view.txtSpecies
import kotlinx.android.synthetic.main.layout_logdetail_item.view.txtVolume
import kotlinx.android.synthetic.main.layout_logentry_detail.*
import kotlinx.android.synthetic.main.layout_logentry_detail.imgBack
...
class LogEntryDetailActivity : AppCompatActivity(), BasicFragment.saveListener {

    override fun saveToDBDone() {
    ...
    //var listener : LogDetailListener ? = null
    var adapter: TabDetailPagerAdapter? = null
    var logID: Int = 0
    var stackNR: String? = null
    var recentTab: String? = null
    var foundNextItem: Boolean = false

    // lateinit var txtVolume : View
    ...
    var currentTabSelected: String? = null
    var mUserVisibilityHint = true
    var currentFilePath: String? = null
    var currentFile: File? = null

    var stacknrValue:String? = null
    var logStackEntityList: List<LogStackEntity>? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.layout_logentry_detail)

        mUserVisibilityHint = false
        ...
        recentTab = intent.getStringExtra("recentTab")
        stackNR = intent.getStringExtra("stackNR")


        val settings = PreferenceManager.getDefaultSharedPreferences(this)
        val editor = settings.edit()
        editor.putString("StackNrValue", stackNR)
        editor.commit()


        currentTabSelected = recentTab
        ...
        logStackEntityList = ArrayList()

        // txtVolume = logStackEntity.txtLogVolume

        logStackEntity.txtVolume.text = Double.toString(0.00)

        //  mLogStackEntity = viewModel.findLogEntityById(logID)

        viewModel?.repository.allNotes.observe(this, Observer<List<LogStackEntity>> { logStackEntityList ->
            ...
            if (logList!!.size == 0) {
                ...
                rlDetailView.visibility = View.GONE
                imgDelEntry.isEnabled = false
            } else {
                rlEmptyView.visibility = View.GONE
                rlDetailView.visibility = View.VISIBLE
                imgDelEntry.isEnabled = true
            }
        ...
        if (logID == 1) {
            //  imgBackwd.isEnabled = false
            imgBackwd.setImageResource(R.drawable.ic_backwrd_disable)
            //imgBackwd.visibility = View.GONE
        } else {
            imgBackwd.isEnabled = true
            imgBackwd.setImageResource(R.drawable.ic_backward)
            imgBackwd.visibility = View.VISIBLE
        }
        ...
        imgBack.setOnClickListener {
            ...
            adapter?.onBackPresssed()
            super.onBackPressed()
        }

        imgForwd.setOnClickListener {

            showForwardLogData()

            /*if(logStackEntity != null){
                logID = logID + 1
                mLogStackEntity =  logStackEntity
                setTitleLayoutData()
                viewPager.adapter = null
                adapter = null
                stackNR = "Polter-"+logID
                adapter = TabDetailPagerAdapter(this, supportFragmentManager, tabLayout!!.tabCount, this)
                viewPager!!.adapter = adapter
                viewPager!!.currentItem = getTabIndex(recentTab!!)
             //   initializeTabData()
                imgForwd.isEnabled = true
                imgForwd.setImageResource(R.drawable.ic_forward)
                imgForwd.visibility = View.VISIBLE

            }else{
                imgForwd.isEnabled = false
                imgForwd.setImageResource(R.drawable.ic_fwd_disable)
                imgForwd.visibility = View.GONE
            }*/

        }

        imgBackwd.setOnClickListener {
            ...
            /*var logStackEntity = viewModel.findLogById(logID-1)
...
*/
        ...
        imgDelEntry.setOnClickListener {

            viewModel.deleteLogEntry(mLogStackEntity!!)
            if (mLogStackEntity!!.logBasicEntity != null)
                viewModel.deleteLogFromBasicTab(mLogStackEntity!!.logBasicEntity!!)
            if (mLogStackEntity!!.logMeasurementEntity != null)
                viewModel.deleteLogFromMeasureTab(mLogStackEntity!!.logMeasurementEntity!!)
            if (mLogStackEntity!!.logPhotoEntity != null)
                viewModel.deleteLogFromPhotoTab(mLogStackEntity!!.logPhotoEntity!!)
            if (mLogStackEntity!!.logLocationEntity != null)
                viewModel.deleteLogFromLocationTab(mLogStackEntity!!.logLocationEntity!!)

            val toBeShownEntity = viewModel.findLogById(logID + 1)
            if (toBeShownEntity != null) {
                showForwardLogData()
            ...
            // viewModel.deleteLogFromPhotoTab(mLogStackEntity!!)
        }


        imgAddNewEntry.setOnClickListener {
            ...
            val alertDialog = AlertDialog.Builder(this)
                    ...
                    .setPositiveButton(getString(R.string.yes)) { dialog, whichButton ->
                        ...
                        var copyFromExitingEntity = logList?.get(logList?.size!! - 1)
                        //copyFromExitingEntity = viewModel?.findLogById(currentLogIndex - 1)
                        ...
                        if (copyFromExitingEntity != null) {
                            ...
                            mLogStackEntity = copyFromExitingEntity
                            setTitleLayoutData()
                            viewPager.adapter = null
                            adapter = null
        ...
        imgExportEntry.setOnClickListener {

            registerForContextMenu(imgExportEntry)
            openContextMenu(imgExportEntry)
        }

    }


    fun setTabBG(firstTab: Int, secondTab: Int, thirdTab: Int, lastTab: Int) {
        //  val firstTabView = tabLayout.getChildAt(0);
        ...
        val thirdTabView = (tabLayout.getChildAt(0) as ViewGroup).getChildAt(2)
        val lastTabView = (tabLayout.getChildAt(0) as ViewGroup).getChildAt(3)

        if (firstTabView != null) {
            setPaddingWithDrawable(firstTabView, firstTab)
        }

        if (secondTabView != null) {
            setPaddingWithDrawable(secondTabView, secondTab)
        }

        if (thirdTabView != null) {
            setPaddingWithDrawable(thirdTabView, thirdTab)
        }

        if (lastTabView != null) {
            setPaddingWithDrawable(lastTabView, lastTab)
        }
    }

    fun setPaddingWithDrawable(view: View, drawable: Int) {

        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.JELLY_BEAN_MR1) {


            val paddingStart = view.getPaddingStart();
            val paddingTop = view.getPaddingTop();
            val paddingEnd = view.getPaddingEnd();
            val paddingBottom = view.getPaddingBottom();
            ViewCompat.setBackground(view, AppCompatResources.getDrawable(view.getContext(), drawable));
    ...
    override fun onStop() {
        ...
        val measurementTabEntity = viewModel.findLogMeasureTabById(logID)
        if (measurementTabEntity != null)
            viewModel.updateLogRecentTab(getTabName(viewPager!!.currentItem), measurementTabEntity!!.surveyingType, logID)
    ...
    fun getTabName(position: Int): String {
        if (position == 0) {
            return getString(R.string.tab_basic)
        } else if (position == 1) {
            return getString(R.string.tab_measurement)
        } else if (position == 2) {
    ...
    fun getTabIndex(tabName: String): Int {
        if (tabName.equals(getString(R.string.tab_basic))) {
            return 0
        } else if (tabName.equals(getString(R.string.tab_measurement))) {
            return 1
        } else if (tabName.equals(getString(R.string.tab_photos))) {
            return 2
        } else {
            return 3
        }
    ...
    fun setTabBackground() {
        if (viewPager!!.currentItem == 0) {
            setTabBG(R.drawable.tab_left_select, R.drawable.tab_unselected, R.drawable.tab_unselected, R.drawable.tab_right_unselect);
        } else if (viewPager!!.currentItem == 1) {
            setTabBG(R.drawable.tab_left_unselect, R.drawable.tab_selected, R.drawable.tab_unselected, R.drawable.tab_right_unselect);
        } else if (viewPager!!.currentItem == 2) {
            setTabBG(R.drawable.tab_left_unselect, R.drawable.tab_unselected, R.drawable.tab_selected, R.drawable.tab_right_unselect);
        } else {
            setTabBG(R.drawable.tab_left_unselect, R.drawable.tab_unselected, R.drawable.tab_unselected, R.drawable.tab_right_select);

        }
    }


    fun setTitleLayoutData() {

        try {

            if (mLogStackEntity != null) {

                logStackEntity.txtLogTitle.text = mLogStackEntity?.logBasicEntity?.stackNR

                logStackEntity.txtDate.text = mLogStackEntity?.logDate
                if (!TextUtils.isEmpty(mLogStackEntity?.logSpeciesCount)) {
                    logStackEntity.txtSpecies.text = resources.getString(R.string.species_text, mLogStackEntity?.logSpeciesCount)
                    logStackEntity.txtSpecies.setCompoundDrawables(null, null, null, null)
                } else {
                    logStackEntity.txtSpecies.text = resources.getString(R.string.species_text, "")
                    logStackEntity.txtSpecies.setCompoundDrawables(null, null, resources.getDrawable(R.drawable.ic_cloudd), null)
                }
                if (!TextUtils.isEmpty(mLogStackEntity?.logKind)) {
                    logStackEntity.txtKind.text = mLogStackEntity?.logKind
                    // logStackEntity.txtKind.setCompoundDrawables(null, null, null, null)
                } else {
                    logStackEntity.txtKind.text = ""
                    // logStackEntity.txtKind.setCompoundDrawables(null, null, resources.getDrawable(R.drawable.ic_cloudd), null)
                }
                if (!TextUtils.isEmpty(mLogStackEntity?.logQuality)) {
                    logStackEntity.txtQuality.text = resources.getString(R.string.quality_text, mLogStackEntity?.logQuality)
                    logStackEntity.txtQuality.setCompoundDrawables(null, null, null, null)
                } else {
                    logStackEntity.txtQuality.text = resources.getString(R.string.quality_text, "")

                    logStackEntity.txtQuality.setCompoundDrawables(null, null, resources.getDrawable(R.drawable.ic_cloudd), null)
                }
                if (!TextUtils.isEmpty(mLogStackEntity?.logLength.toString())) {
                    logStackEntity.txtLength.text = resources.getString(R.string.length_text, Double.toString(mLogStackEntity?.logLength!!))
                    logStackEntity.txtLength.setCompoundDrawables(null, null, null, null)
                } else {
                    logStackEntity.txtLength.text = resources.getString(R.string.length_text, "")

                    logStackEntity.txtLength.setCompoundDrawables(null, null, resources.getDrawable(R.drawable.ic_cloudd), null)
                }

                if (mLogStackEntity?.logBasicEntity?.iosID != null) {

                    logStackEntity.txtForestOwner.text = resources.getString(R.string.los_id_text, mLogStackEntity?.logBasicEntity?.iosID)

                } else {
                    logStackEntity.txtForestOwner.text = resources.getString(R.string.los_id_text, " ")

                }

                logStackEntity.txtLogVolume.text = resources.getString(R.string.log_volume, Double.toString(mLogStackEntity?.logMeasurementEntity?.logsEntity?.total_volume_m3!!))

                logStackEntity.txtVolume.text = Double.toString(mLogStackEntity?.logMeasurementEntity?.sectionEntity?.m3!!)


                var surveyType = getString(R.string.logs_surveying)
                if (mLogStackEntity?.logMeasurementEntity != null) {
                    surveyType = mLogStackEntity?.logMeasurementEntity?.surveyingType!!
                }
                if (surveyType.equals(getString(R.string.logs_surveying))) {
                    logStackEntity.llSpeciesLayout.visibility = View.GONE
                    logStackEntity.llLengthLayout.visibility = View.GONE
                    logStackEntity.llLogLayout.visibility = View.VISIBLE
                    logStackEntity.txtDate.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ic_log, 0, 0, 0)
                } else {
                    logStackEntity.llSpeciesLayout.visibility = View.VISIBLE
                    //   logStackEntity.llLengthLayout.visibility = View.VISIBLE
                    logStackEntity.llLogLayout.visibility = View.GONE
                    if (surveyType.equals(getString(R.string.section_surveying))) {
                        logStackEntity.txtDate.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ic_section, 0, 0, 0)
                    } else {
                        logStackEntity.txtDate.setCompoundDrawablesWithIntrinsicBounds(R.drawable.ic_estimation, 0, 0, 0)

                    }
                }

                logStackEntity.txtLogCount.text = getString(R.string.log_count_homescreen, mLogStackEntity?.logMeasurementEntity?.logsEntity?.log_count.toString())
                /*if(mLogStackEntity?.logMeasurementEntity?.logsEntity?.total_volume_m3.toString() != null){
                    logStackEntity.txtVolume.text = getString(R.string.log_volume,mLogStackEntity?.logMeasurementEntity?.logsEntity?.total_volume_m3.toString())
                }else{
                    logStackEntity.txtVolume.text = getString(R.string.log_volume,"0.00")
                }*/

                //   logStackEntity.txtVolume.text = getString(R.string.log_volume,Double.toString(mLogStackEntity?.logMeasurementEntity?.logsEntity?.total_volume_m3!!))
            }
    ...
    fun initializeTabData() {

        tabLayout.addTab(tabLayout.newTab().setText(getString(R.string.tab_basic)))
        tabLayout!!.addTab(tabLayout!!.newTab().setText(getString(R.string.tab_measurement)))
        tabLayout!!.addTab(tabLayout!!.newTab().setText(getString(R.string.tab_photos)))
        tabLayout!!.addTab(tabLayout!!.newTab().setText(getString(R.string.tab_location)))
        ...
        tabLayout!!.addOnTabSelectedListener(object : TabLayout.OnTabSelectedListener {
            override fun onTabSelected(tab: TabLayout.Tab) {
                ...
                if (tabLayout.getSelectedTabPosition() == 0) {
                    currentTabSelected = getString(R.string.tab_basic)
                    setTabBG(R.drawable.tab_left_select, R.drawable.tab_unselected, R.drawable.tab_unselected, R.drawable.tab_right_unselect);
                } else if (tabLayout.selectedTabPosition == 1) {
                    currentTabSelected = getString(R.string.tab_measurement)
                    setTabBG(R.drawable.tab_left_unselect, R.drawable.tab_selected, R.drawable.tab_unselected, R.drawable.tab_right_unselect);
                } else if (tabLayout.selectedTabPosition == 2) {
                    currentTabSelected = getString(R.string.tab_photos)
                    setTabBG(R.drawable.tab_left_unselect, R.drawable.tab_unselected, R.drawable.tab_selected, R.drawable.tab_right_unselect);
                } else {
                    currentTabSelected = getString(R.string.tab_location)
                    setTabBG(R.drawable.tab_left_unselect, R.drawable.tab_unselected, R.drawable.tab_unselected, R.drawable.tab_right_select);
    ...
    fun showForwardLogData() {

        var logStackEntity: LogStackEntity? = null
        if (currentIndex + 1 < logList?.size!!) {
            imgForwd.visibility = View.VISIBLE
            logStackEntity = logList?.get(currentIndex + 1)
        } else {
    ...
    fun insertNewLogStack() {
        ...
        val basicEntity = BasicTabEntity(currentLogIndex, null, logTitle,
                null, null, 0.00,
        ...
        when (item?.getItemId()) {
            ...
            Utilities.EXPORT_CREATE_PDF -> {
                ...
                if (BuildConfig.PAID_VERSION) {
                    val isSuccess = createAndDisplayPdf(mLogStackEntity!!)
                    if (isSuccess) {
            ...
            Utilities.EXPORT_SEND_PDF -> {
                ...
                if (BuildConfig.PAID_VERSION) {
                    ...
                    if (isSuccess) {
                        ...
                        shareIntent.putExtra(Intent.EXTRA_SUBJECT, "Log Stack - " + mLogStackEntity?.logTitle)
    ...
    fun createAndDisplayPdf(entity: LogStackEntity): Boolean {
    ...
    private fun viewCSV(files: File, file: String, directory: String) {
        ...
        csvIntent.setDataAndType(contentUri, "application/csv")
        ...
        try {
            if (csvIntent.resolveActivity(packageManager) != null) {
                startActivity(csvIntent)
            } else {
                Toast.makeText(this, "No app found for opening csv file", Toast.LENGTH_LONG).show()
            }
    ...
    private fun viewExcel(files: File, file: String, directory: String) {
        ...
        try {
            if (csvIntent.resolveActivity(packageManager) != null) {
                ...
                Toast.makeText(this, "No app found for opening excel file", Toast.LENGTH_LONG).show()
            }
